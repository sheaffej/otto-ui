    ...
    getLogsObservable(): Observable<EngineLogEntry> {
        ...
        if (this.observable == null) {
            ...
            this.observable = this.http.get<OttoRestResponse>(`${this.ottoRestUrl}/logs`)
                .pipe(
                    // The event has a single object with a "data" attribute holding an array
                    map(resp => resp.data),
                    ...
                    map(logs => logs.sort((a: EngineLogEntry, b: EngineLogEntry) => {
                            return a.ts.getTime() - b.ts.getTime();
                    })),
                    // Now we have a sorted array based on timestamp (ascending)
                    map(logs => logs.reverse()),
                    // Now we reversed the array so it's sorted by descending timestamp
                    flatMap(logs => from(logs)),
                    // flatMat converst the single event with an array, to many events
            );
        }
